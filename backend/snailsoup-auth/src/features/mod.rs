...
pub fn get_routes(app_state: AppState) -> Router {
    ...
    let admin_routes = user::api::get_admin_routes(app_state.clone()).route_layer(
        axum::middleware::from_fn_with_state(app_state.clone(), auth::middleware::authorize_admin),
    );
